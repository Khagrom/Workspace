 ...
 * @author Fanny Rottee
 */
public class Rectangle2D implements FormeGeometrique2D, Cloneable {
    ...
    public String toString() {
        return "Rectangle formé par les points "
                + s1.getP1().toString() + ", "
                + s2.getP1().toString() + ", "
                + s3.getP1().toString() + " et "
                + s4.getP1().toString() + ".";
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!s3.equals(other.s3)) {
            return false;
        }

        return s4.equals(other.s4);
    }

    @Override
    public void deplace(double dx, double dy) {
        Segment2D[] tab = {s1, s2, s3, s4};
        for (Segment2D s : tab) {
            s.deplace(dx, dy);
        }
    }
    ...
    @Override
    public Rectangle2D clone() throws CloneNotSupportedException {
        return new Rectangle2D(s1.getP1(), s2.getP1(), s3.getP1(), s4.getP1());
    }
}
